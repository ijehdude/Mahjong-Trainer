...
for (const d of ["beginner", "intermediate", "advanced"] as const) {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    if (q.options && q.options.length !== 4 && q.type === "strategy")
      throw new Error(`strategy q has ${q.options.length} options, want 4`);
    if (q.options && new Set(q.options.map((o) => o.id)).size !== q.options.length)
    ...
    if (q.type === "strategy") for (const c of q.correct) stratIds.add(c);
  }
  console.log(d, JSON.stringify(counts), `${((performance.now() - t0) / 50).toFixed(0)}ms/question`);
}
console.log("strategy correct-answers seen:", [...stratIds].sort().join(", "));
