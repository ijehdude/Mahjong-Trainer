...
/* ===========================================================================
   ...
   `count` tiles fit one row — capped at `max`, floored at `min` (below which
   the row scrolls horizontally instead of wrapping). The size is exposed as
   --tile-w/--tile-h CSS variables consumed by TileComponent's "scaled" size.
   Aspect ratio is kept at 42:58.
   =========================================================================== */
...
export function useScaledTileRow(
  ...
  const { min = 34, max = 48, gap = 4, extra = 0 } = opts ?? {};
